...
// for cylindrical partition

__global__ void cylinder_kernel_hash_kernel(int N, int K, const int *__restrict__ data,
                                   const int *__restrict__ kernel_offset,
                                   int64_t *__restrict__ out, int max_theta) {
  extern __shared__ int kernel_offset_local[];
  ...
  if (i < N) {
    ...
    for (int j = 0; j < 3; j++) {
      cur_coord[j] = data[j] + kernel_offset[k * 3 + j]; 
      if (j == 1) {
          cur_coord[j] %= max_theta; //assert 0 <= theta < max_theta
      }
    }
    cur_coord[3] = data[3];
    uint64_t hash = 14695981039346656037UL;
...
void cylinder_kernel_hash_wrapper(int N, int K, const int *data,
                         const int *kernel_offset, int64_t *out, int max_theta) {
  cylinder_kernel_hash_kernel<<<ceil((double)(N * K) / 512), 512, K * 3 * sizeof(int)>>>(
      N, K, data, kernel_offset, out, max_theta);
}
...
at::Tensor cylinder_kernel_hash_cuda(const at::Tensor idx,
  ...
  int N = idx.size(0);
  int K = kernel_offset.size(0);
  int max_theta = max_theta;
  at::Tensor out = torch::zeros(
      {K, N}, at::device(idx.device()).dtype(at::ScalarType::Long));
  cylinder_kernel_hash_wrapper(N, K, idx.data_ptr<int>(), kernel_offset.data_ptr<int>(),
                      out.data_ptr<int64_t>(), max_theta);
  return out;
}
