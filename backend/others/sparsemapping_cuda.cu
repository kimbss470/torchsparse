...
__host__ __device__ inline int64_t transform_coords(int64_t *in_coords,
                                                    int64_t *coords_min,
                                                    int64_t *coords_max) {
  int64_t cur = 0;
  int64_t sizes[NDim];
#pragma unroll
  for (int64_t i = 0; i < NDim; i++) sizes[i] = coords_max[i] - coords_min[i] + 1;
#pragma unroll
...
__host__ __device__ int64_t get_output_coords_downsample(
    int64_t kernel_volume, int64_t *in_coords, int64_t *kernel_sizes, int64_t *stride,
    int64_t *tensor_stride, int64_t *coords_min, int64_t *coords_max, int64_t *out_coords,
    int64_t *offsets) {
  int64_t point_counter = 0;
  int64_t upper[NDim], lower[NDim], counter[NDim], cur;
  bool valid = false;
#pragma unroll
  for (int64_t i = 0; i < NDim; ++i) {
    counter[i] = 0;
...
__host__ __device__ int64_t get_output_coords_subm(int64_t kernel_volume,
                                               int64_t *in_coords,
                                               int64_t *kernel_sizes,
                                               int64_t *coords_min, int64_t *coords_max,
                                               int64_t *out_coords, int64_t *offsets) {
  int64_t point_counter = 0;
  int64_t upper[NDim], lower[NDim], counter[NDim], cur;
#pragma unroll
  for (int64_t i = 0; i < NDim; ++i) {
...
__global__ void inverse_transform_coords_kernel(int64_t n_points,
                                                int64_t *in_coords,
                                                int64_t *coords_min,
                                                int64_t *coords_max,
                                                int64_t *out_coords) {
...
__global__ void downsample_grid_kmap_stage1_specialized(
    int64_t n_points, int64_t n_kernel, int64_t *in_coords, int64_t *kernel_sizes, int64_t *stride,
    int64_t *tensor_stride, int64_t *coords_min, int64_t *coords_max, int64_t *n_out_points,
    int64_t *transformed_coords, int64_t *out_kmap, int64_t *kmap_sizes) {
  int64_t idx = blockIdx.x * blockDim.x + threadIdx.x;
  // a little bit more complicated. need to consider offset.
  if (idx >= n_points) return;
  int64_t cnt = 0;
  int64_t coords_out[NDim], coords_out_tmp[NDim];
  coords_out[0] = in_coords[idx * NDim];
  coords_out_tmp[0] = in_coords[idx * NDim];
#pragma unroll
  for (int64_t i = -(kernel_sizes[0] - 1) / 2; i <= (kernel_sizes[0] - 1) / 2;
       i++) {
    coords_out[1] = in_coords[idx * NDim + 1] / tensor_stride[0] + i;
#pragma unroll
    for (int64_t j = -(kernel_sizes[1] - 1) / 2; j <= (kernel_sizes[1] - 1) / 2;
         j++) {
      coords_out[2] = in_coords[idx * NDim + 2] / tensor_stride[1] + j;
#pragma unroll
      for (int64_t k = -(kernel_sizes[2] - 1) / 2; k <= (kernel_sizes[2] - 1) / 2;
           k++) {
        ...
        if (coords_out_tmp[1] >= coords_min[1] &&
          ...
          int64_t old_num = atomicAdd(&kmap_sizes[cnt], 1);
          int64_t old_idx = atomicAdd(n_out_points, 1);
...
__global__ void downsample_grid_kmap_stage1(
  ...
  // a little bit more complicated. need to consider offset.
  if (idx >= n_points) return;
  int64_t coords_out[MAX_KVOL * NDim], offsets[MAX_KVOL], local_coords_min[NDim],
      local_coords_max[NDim];
  int64_t point_counter = get_output_coords_downsample(
...
#pragma unroll
    ...
    // out idx.
    int64_t old_num = atomicAdd(&kmap_sizes[offset], 1);
...
__global__ void downsample_grid_kmap_stage2(int64_t n_points, int64_t *in_coords,
                                            int64_t *out_grids) {
  int64_t idx = blockIdx.x * blockDim.x + threadIdx.x;
  if (idx >= n_points) return;
...
__global__ void downsample_grid_kmap_stage3(int64_t n_points, int64_t n_points_out,
                                            int64_t kernel_volume, int64_t *grids,
                                            int64_t *in_kmap, int64_t *kmap_sizes,
                                            int64_t *cum_kmap_sizes, int64_t *out_kmap,
                                            int64_t *input_mask, int64_t *output_mask) {
  int64_t idx = blockIdx.x * blockDim.x + threadIdx.x;
  if (idx >= n_points) return;
  for (int64_t i = 0; i < kernel_volume; i++) {
    int64_t kmap_size = kmap_sizes[i];
    int64_t cum_size = i == 0 ? 0 : cum_kmap_sizes[i - 1];
    int64_t *cur_in_kmap = in_kmap + 2 * n_points * i;
    if (idx >= kmap_size) continue;
    // manual unroll
    int64_t input_idx = (int64_t)cur_in_kmap[idx * 2];
    int64_t output_idx = (int64_t)grids[cur_in_kmap[idx * 2 + 1]];
...
__global__ void subm_grid_kmap_stage1(int64_t n_points, int64_t kernel_volume,
                                      int64_t *in_coords, int64_t *coords_min,
                                      int64_t *coords_max, int64_t *out_grids) {
  int64_t idx = blockIdx.x * blockDim.x + threadIdx.x;
  if (idx >= n_points) return;
  int64_t grid_index =
...
__global__ void subm_grid_kmap_stage2(int64_t n_points, int64_t kernel_volume,
                                      int64_t *in_coords, int64_t *coords_min,
                                      int64_t *coords_max, int64_t *kernel_sizes,
                                      int64_t *stride, int64_t *tensor_stride,
                                      int64_t *grids, int64_t *out_kmap,
                                      int64_t *kmap_sizes) {
  int64_t idx = blockIdx.x * blockDim.x + threadIdx.x;
  if (idx >= n_points) return;
  /*
  int64_t coords_out[MAX_KVOL * NDim], offsets[MAX_KVOL];

  int64_t n_neighbors = get_output_coords_subm(
    kernel_volume, in_coords + idx * NDim, kernel_sizes,
    coords_min, coords_max, coords_out, offsets
  );
#pragma unroll
  for(int64_t i = 0; i < n_neighbors / 2; i++){
    int64_t grid_index = transform_coords(coords_out + i * NDim, coords_min,
coords_max); int64_t input_idx = grids[grid_index]; if(input_idx != -1){ int64_t old_num
= atomicAdd(&kmap_sizes[i], 1); atomicAdd(&kmap_sizes[kernel_volume-1-i], 1);
  ...
  int64_t old_num = atomicAdd(&kmap_sizes[n_neighbors / 2], 1);
  //out_kmap[((i * n_points) << 1) + (old_num << 1)] = idx;
  //out_kmap[((i * n_points) << 1) + (old_num << 1) + 1] = idx;
  */
  int64_t cnt = 0;
  int64_t coords_out[NDim];
  coords_out[0] = in_coords[idx * NDim];
#pragma unroll
  for (int64_t i = -(kernel_sizes[0] - 1) / 2; i <= (kernel_sizes[0] - 1) / 2;
       i++) {
      ...
      for (int64_t k = -(kernel_sizes[2] - 1) / 2; k <= (kernel_sizes[2] - 1) / 2;
        ...
        if (cnt == kernel_volume / 2) {
          ...
          if (coords_out[1] >= coords_min[1] &&
            ...
            int64_t input_idx = grids[grid_index];
            if (input_idx != -1) {
              int64_t old_num = atomicAdd(&kmap_sizes[cnt], 1);
...
__global__ void subm_grid_kmap_stage3(int64_t n_points, int64_t kernel_volume,
                                      int64_t *in_kmap, int64_t *kmap_sizes,
                                      int64_t *cum_kmap_sizes, int64_t *out_kmap,
                                      int64_t *input_mask, int64_t *output_mask) {
  int64_t idx = blockIdx.x * blockDim.x + threadIdx.x;
  ...
  for (int64_t i = 0; i < kernel_volume; i++) {
    ...
    // manual unroll
    int64_t input_idx = (int64_t)cur_in_kmap[idx * 2];
    int64_t output_idx = (int64_t)cur_in_kmap[idx * 2 + 1];
    ...
    */
    ...
    // int64_t cum_size2 = cum_kmap_sizes[kernel_volume - 1 - i - 1];
    // int64_t *cur_in_kmap2 = in_kmap + 2 * n_points * (kernel_volume - 1 -i);
...
// replace the coords in the output map with the output idx
__global__ void get_masks_from_kmap_kernel(int64_t n_points, int64_t n_points_out,
                                           int64_t kernel_volume, int64_t *kmap,
                                           int64_t *kmap_sizes, int64_t *cum_kmap_sizes,
                                           int64_t *input_mask, int64_t *output_mask) {
  int64_t idx = blockIdx.x * blockDim.x + threadIdx.x;
  if (idx >= n_points) return;
  for (int64_t i = 0; i < kernel_volume; i++) {
    ...
    int64_t kmap_size = kmap_sizes[i];
    int64_t cum_size = i == 0 ? 0 : cum_kmap_sizes[i - 1];
    int64_t *cur_in_kmap = kmap + cum_size * 2;
    if (idx >= kmap_size) continue;
    // manual unroll
    int64_t input_idx = cur_in_kmap[idx * 2];
    int64_t output_idx = cur_in_kmap[idx * 2 + 1];
    // another layout
    input_mask[i * n_points + input_idx] = idx;
    output_mask[i * n_points_out + output_idx] = idx;
  }
...
std::vector<at::Tensor> build_kernel_map_subm(
  ...
  int64_t *in_coords = _in_coords_tmp.data_ptr<int64_t>();
  int64_t *coords_min = _coords_min_tmp.data_ptr<int64_t>();
  int64_t *coords_max = _coords_max_tmp.data_ptr<int64_t>();
  int64_t *kernel_sizes = _kernel_sizes.data_ptr<int64_t>();
  int64_t *stride = _stride.data_ptr<int64_t>();
  int64_t *tensor_stride = _tensor_stride.data_ptr<int64_t>();
  ...
  at::Tensor _grids = torch::full(
      {(int64_t)(torch::prod((_coords_max - _coords_min) / augmented_tensor_stride +
                         1)
                 .item<int64_t>())},
      -1, options);
  int64_t *grids = _grids.data_ptr<int64_t>();
  at::Tensor _out_kmap =
  ...
  int64_t *kmap_sizes = _kmap_sizes.data_ptr<int64_t>();
  int64_t *input_mask = _input_mask.data_ptr<int64_t>();
  int64_t *output_mask = _output_mask.data_ptr<int64_t>();
  // stage1: insert to grid
  subm_grid_kmap_stage1<<<(int64_t)ceil((double)n_points / 256), 256>>>(
      n_points, kernel_volume, in_coords, coords_min, coords_max, grids);
  // stage2: query
  subm_grid_kmap_stage2<<<(int64_t)ceil((double)n_points / 256), 256>>>(
  ...
  at::Tensor final_out_kmap =
      torch::zeros({(int64_t)(torch::sum(_kmap_sizes).item<int64_t>()), 2}, options);
  at::Tensor _cum_kmap_sizes =
      torch::cumsum(_kmap_sizes, 0).to(at::ScalarType::Int);
  int64_t *cum_kmap_sizes = _cum_kmap_sizes.data_ptr<int64_t>();
  subm_grid_kmap_stage3<<<
      (int64_t)ceil((double)(torch::max(_cum_kmap_sizes).item<int64_t>()) / 256),
      256>>>(n_points, kernel_volume, out_kmap, kmap_sizes, cum_kmap_sizes,
             final_out_kmap.data_ptr<int64_t>(), input_mask, output_mask);
...
std::vector<at::Tensor> build_kernel_map_downsample(
  ...
  int64_t n_points = _in_coords.size(0);
  int64_t kernel_volume = (int64_t)(torch::prod(_kernel_sizes).item<int64_t>());
  int64_t *in_coords = _in_coords.data_ptr<int64_t>();
  int64_t *coords_min = _coords_min.data_ptr<int64_t>();
  int64_t *coords_max = _coords_max.data_ptr<int64_t>();
  int64_t *kernel_sizes = _kernel_sizes.data_ptr<int64_t>();
  int64_t *stride = _stride.data_ptr<int64_t>();
  int64_t *tensor_stride = _tensor_stride.data_ptr<int64_t>();
  auto options = torch::TensorOptions()
  ...
  int64_t *out_kmap = _out_kmap.data_ptr<int64_t>();
  int64_t *kmap_sizes = _kmap_sizes.data_ptr<int64_t>();
  int64_t *n_out_points = _n_out_points.data_ptr<int64_t>();
  ...
  */
  ...
  downsample_grid_kmap_stage1<<<(int64_t)ceil((double)n_points / 256),
  ...
  */
  ...
  at::Tensor _coords_max_tmp =
      (_coords_max / augmented_tensor_stride).to(at::ScalarType::Int);
  coords_min = _coords_min_tmp.data_ptr<int64_t>();
  coords_max = _coords_max_tmp.data_ptr<int64_t>();

  /*
  // If we do specialized downsample for 3D coords (stage 1), we do it (using
  divided coords_min/max) as follows:
  */
  downsample_grid_kmap_stage1_specialized<<<(int64_t)ceil((double)n_points / 256),
                                            256>>>(
  ...
  at::Tensor _grids = torch::full(
      {(int64_t)(torch::prod(_coords_max_tmp - _coords_min_tmp + 1).item<int64_t>())},
      -1, options);
  int64_t *grids = _grids.data_ptr<int64_t>();
  ...
  int64_t n_out_points_scalar = _out_coords.size(0);
  at::Tensor final_out_coords =
      torch::zeros({n_out_points_scalar, NDim}, options);
  inverse_transform_coords_kernel<<<
      (int64_t)ceil((double)n_out_points_scalar / 256), 256>>>(
      n_out_points_scalar, out_coords, coords_min, coords_max,
      final_out_coords.data_ptr<int64_t>());
  // restore coords
  final_out_coords *= augmented_tensor_stride;

  downsample_grid_kmap_stage2<<<(int64_t)ceil((double)n_out_points_scalar / 256),
  ...
  downsample_grid_kmap_stage3<<<
      (int64_t)ceil((double)torch::max(_cum_kmap_sizes).item<int64_t>() / 256), 256>>>(
      n_points, n_out_points_scalar, kernel_volume, grids, out_kmap, kmap_sizes,
      cum_kmap_sizes, final_out_kmap.data_ptr<int64_t>(), input_mask, output_mask);
...
std::vector<at::Tensor> build_mask_from_kmap(int64_t n_points, int64_t n_out_points,
  ...
  int64_t kernel_volume = _kmap_sizes.size(0);
  auto options =
  ...
  int64_t *kmap = _kmap.data_ptr<int64_t>();
  int64_t *kmap_sizes = _kmap_sizes.data_ptr<int64_t>();
  int64_t *cum_kmap_sizes = _cum_kmap_sizes.data_ptr<int64_t>();
  int64_t *input_mask = _input_mask.data_ptr<int64_t>();
  int64_t *output_mask = _output_mask.data_ptr<int64_t>();

  int64_t max_kmap_size = 1;
  if (kernel_volume % 2 == 1 && n_points == n_out_points) {
    max_kmap_size =
        *std::max_element(_kmap_sizes_cpu.data_ptr<int64_t>(),
                          _kmap_sizes_cpu.data_ptr<int64_t>() + kernel_volume / 2);
    max_kmap_size =
        std::max(max_kmap_size,
                 *std::max_element(
                     _kmap_sizes_cpu.data_ptr<int64_t>() + kernel_volume / 2 + 1,
                     _kmap_sizes_cpu.data_ptr<int64_t>() + kernel_volume));
