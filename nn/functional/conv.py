...
def conv3d(
    ...
    # @kimbs
    statistics.layer_id += 1
    statistics.stat_voxel_count(coords)
    statistics.stat_sparsity(feats)
    statistics.stat_feat_size(feats, kernel_size, stride)

    #  statistics.store_metadata(input.stride, kernel_size, stride)
    ...
    if kernel_size == (1,1,1):
        statistics.stat_MAC_count(weight, torch.tensor([coords.size(0)]), coords.size(0), coords.size(0))
    elif transposed:
        statistics.stat_MAC_count(weight, kmap[1].clone(), kmap[2][1], kmap[2][0], transposed)
    else:
        statistics.stat_MAC_count(weight, kmap[1].clone(), kmap[2][0], kmap[2][1], transposed)
